...
import { Calendar, Clock, ArrowLeft, TrendingUp, TrendingDown, Activity, AlertTriangle, ShieldAlert, Zap, BrainCircuit, Lock } from 'lucide-react';
...
// 3. Pagina Universală (Compatibilă Next.js 15)
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params; // Aici e cheia: await params
  const article = articles.find((a) => a.slug === slug);
  ...
  // Configurare Temă Vizuală bazată pe Impact
  const getTheme = () => {
      ...
      <article className="flex-1 container mx-auto px-6 py-12 max-w-4xl">
        ...
        <div className="mt-16 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 font-[var(--font-inter)]">
            ...
            <div className="flex flex-col gap-2 w-full md:w-auto">
                <span className="text-xs text-gray-500 uppercase tracking-widest font-bold">Distribuie:</span>
        ...
        <div className={`mt-12 bg-gradient-to-r ${theme.bg.replace('bg-', 'from-').replace('/10', '/20')} to-gray-900/50 p-8 rounded-2xl border ${theme.border} text-center`}>
            <h3 className="text-2xl font-bold text-white mb-2 font-[var(--font-space)]">
                {article.impact === 'bearish' ? 'Nu pierde bani din neștiință' : 'Pregătește-te pentru oportunitate'}
            </h3>
            <p className="text-gray-300 mb-6 max-w-xl mx-auto">
                {article.impact === 'bearish' 
                    ? 'Piețele volatile sunt periculoase pentru cei fără plan. Securizează-ți portofoliul acum.' 
                    : 'Când piața crește, trebuie să știi când să intri și când să ieși. Hai să facem strategia împreună.'}
            </p>
            <Link href="/#consultanta" className={`inline-flex items-center gap-2 bg-${theme.color}-600 hover:bg-${theme.color}-500 text-white font-bold px-8 py-3 rounded-xl transition-all shadow-lg transform hover:-translate-y-1`}>
                <Zap size={18}/> Rezervă Sesiunea de Strategie ($250)
            </Link>
        </div>
