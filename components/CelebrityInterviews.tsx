...
const VIDEOS = [
  { id: 'e6fT2y3DkqI', title: '💥 Ben Zhou, Bybit Founder - Bear Market Is Over! 🌟 Interviu cu fondatorul Bybit' },
  { id: '59HJsgv8lbI', title: 'Super Interviu cu Rohit Wad (Binance CTO) - Directorul de Tehnologie Binance' },
  { id: 'IMNUP11X93w', title: "Binance's Top Manager Kyrylo Khomiakov (Regional Head CEE) - Interviu Exclusiv" },
  { id: 'cQx2EFPN2ZI', title: 'Mike Silagadze, Founder and CEO of Ether.fi - Restaking Revolution' },
  { id: 'BJ-qiaGsIaY', title: 'Podcast - Dmitry Buterin (Vitalik Buterin Father) - Ethereum Roots' },
  { id: 'CdBYX3NPfbA', title: 'Bitcoin va valora 5.000.000$! Asta este părerea lui MMCrypto' },
  { id: 'SVcy0DJEuoQ', title: 'Usual CEO Pierre Person interview just after project got listed on Binance' },
  { id: 'zcZ-SKhJhqM', title: 'Super Podcast cu Davinci Jeremie - Bitcoin OG' },
];

const VideoCard = ({ id, title }: { id: string; title: string }) => {
  const [isPlaying, setIsPlaying] = useState(false);
  const t = useTranslations('home.interviews');
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-12">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-purple-500/30 bg-purple-500/10 text-purple-300 text-xs font-bold tracking-widest mb-4 uppercase shadow-lg shadow-purple-500/10">
                <Star size={14} className="text-yellow-400 fill-yellow-400"/> {t('badge')}
            </div>
            <h2 className="text-3xl md:text-5xl font-bold mb-4 text-white">
                {t('titleBefore')} <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400">{t('titleHighlight')}</span>
            </h2>
