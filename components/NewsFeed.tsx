...
import { TrendingUp, TrendingDown, Minus, BrainCircuit, ArrowUpRight, Calendar } from 'lucide-react';
import Link from 'next/link';
import { articles } from '@/lib/articles'; // Importăm baza de date

export default function NewsFeed() {
  // Luăm doar primele 3 știri pentru Homepage
    ...
    <section id="news" className="py-24 container mx-auto px-6 relative border-t border-white/5 bg-[#020617]">
        ...
        <div className="text-center mb-16">
            ...
            <p className="text-gray-400 text-lg max-w-2xl mx-auto">
                Nu doar știri. <span className="text-white font-bold">Interpretarea lor.</span><br/>
                Înțelege impactul real asupra portofoliului tău.
            </p>
        </div>
                ...
                <Link href={`/stiri/${item.slug}`} key={idx} className="group relative bg-[#0a0f1e] border border-white/10 rounded-2xl overflow-hidden hover:border-blue-500/30 transition-all hover:-translate-y-1 flex flex-col">
                    ...
                    <div className="p-6 bg-blue-900/5 mt-auto">
                        <div className="flex items-center gap-2 mb-2 text-blue-400 text-xs font-bold uppercase tracking-widest">
                            <BrainCircuit size={14}/> Mihai's Take
                        </div>
                        <p className="text-sm text-gray-300 italic border-l-2 border-blue-500/50 pl-3 line-clamp-3">
                            "{item.mihaiTake}"
                        </p>
                    </div>
                </Link>
        ...
        <div className="mt-12 text-center">
            <Link href="/stiri" className="px-8 py-3 bg-white/5 border border-white/10 rounded-xl text-white font-medium hover:bg-white/10 transition-all">
                Vezi Arhiva de Analize
            </Link>
        </div>
    </section>
  );
